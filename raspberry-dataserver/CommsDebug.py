...
async def commsDebug():
    ...
    cmd = CommandFormat(cmd_type=CMD_TYPE.SET_DURATION.value, cmd_code=CMD_SET_DURATION.INHALE.value, param=3000.) #
    comms.writePayload(cmd)
    await asyncio.sleep(1)
    ...
    cmd = CommandFormat(cmd_type=CMD_TYPE.SET_DURATION.value, cmd_code=CMD_SET_DURATION.EXHALE.value, param=3000.) #
    comms.writePayload(cmd)
    await asyncio.sleep(1)
    # Start the cycles
    cmd = CommandFormat(cmd_type=CMD_TYPE.GENERAL.value, cmd_code=CMD_GENERAL.START.value, param=0)

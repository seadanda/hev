...
class CommsFormat:
    ...
    escapeChar: ClassVar[int] = bytes([0x7D])
    ...
    def __init__(self, info_size=0, address=0x00, control=[0x00, 0x00]):
        ...
        self.assignBytes(self.getStart()  , self.separator  , calc_crc = False)
        ...
        self.assignBytes(self.getStop()   , self.separator  , calc_crc = False)
    ...
    def encode(self):
        data = self._data
        stream = [[int.from_bytes(self.escapeChar, 'little'), byte ^ (1<<5)] if bytes([byte]) == self.escapeChar or bytes([byte]) == self.separator else [byte] for byte in data[1:-1]]
...
class CommsPacket(object):
    ...
    def undoEscape(self):
        ...
        indRemove = [idx for idx in range(len(packets)) if bytes([packets[idx]]) == self.escapeChar]
